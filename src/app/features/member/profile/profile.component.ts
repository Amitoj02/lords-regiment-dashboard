...
export class ProfileComponent implements OnInit {
  ...
  medals = [
    { letter: 'V', ribbon: 'gold' as const, title: 'Valour Cross', description: 'Exceptional battlefield conduct.' },
    { letter: 'C', ribbon: 'tricolor' as const, title: 'Campaign Star', description: 'Completed a full campaign.' },
    { letter: 'L', ribbon: 'red' as const, title: 'Long Service', description: 'Three+ seasons of active service.' },
    { letter: 'D', ribbon: 'blue' as const, title: 'Distinguished Drill', description: 'Excellence in formation drill.' },
  ];
  ...
  ngOnInit(): void {
    ...
    this.membersService.getById(id).subscribe(member => {
      this.member = member ?? null;
    });

    this.galleryService.getAll().subscribe(items => {
      this.galleryItems = items.filter(i => i.submittedBy && i.status === 'approved').slice(0, 6);
      this.taggedItems = items.filter(i => i.taggedMembers?.includes(id)).slice(0, 6);
    });
