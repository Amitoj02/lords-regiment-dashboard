...
matching `sizeWeight`, an `area` (in a monorepo: the package name), likely
`paths`, `dependsOn` relationships (cross-repo: `"<repoKey>:T-####"`), and a
concrete `testPlan`. Allocate fresh non-colliding `T-####` ids.
